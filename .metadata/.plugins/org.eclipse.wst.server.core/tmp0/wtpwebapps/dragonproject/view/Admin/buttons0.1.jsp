...
<div class="card shadow mb-4">
    ...
    <tbody>
        <form action="ANEWSWrite.do" method="post" encType="multiplart/form-data">
            <tr>
				<th>제목</th>
				<th>내용</th>
				<th>날짜</th>
			...
			<tr>
					<td class="dot"><%=lists.get(i).getRELEASES_TITLE() %></td>
					<td class="txt dot"><%=lists.get(i).getRELEASES_CONTENT() %></td>
					<td>class="date"><%= lists.get(i).getRELEASES_DAY() %></td>>

					<td class="imgZone"><img src="<%= lists.get(i).getRELEASES_IMG() %>" alt="tvN 토일 드라마 ‘호텔 델루나’ 자체 최고 기록으로 시청률 10% 돌파! 썸네일 이미지" /></td>
			</tr>
				<%} %>
        </form>
    </tbody>
