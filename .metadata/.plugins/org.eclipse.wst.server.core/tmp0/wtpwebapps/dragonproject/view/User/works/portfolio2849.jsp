...
<!-- Mirrored from www.studiodragon.net/front/kr/works/portfolio?class1=G by HTTrack Website Copier/3.x [XR&CO'2014], Thu, 22 Aug 2019 10:25:13 GMT -->
...
<head>
	...
	<meta name="description" content="아시아의 콘텐츠와 라이프 스타일을 선도하는 Asia No.1 Studio 스튜디오 드래곤의 웰메이드 콘텐츠를 소개합니다.">
	<meta name="keywords" content="스튜디오 드래곤, CJ, CJ E&M, CJ 스튜디오 드래곤, STUDIO DRAGON, 드라마, 콘텐츠, 방영, 종영, 글로벌">
...
<div id="wrap">
		...
		<div class="mobile-wrap">
			<div id="gnb" class=""><!--gng 오버했을때 class open 추가 -->
				<ul>
					...
					<li class="on"><a href="portfolio.jsp" class="menu1 transit">WORKS</a>
						<ul>
							<li class="on"><a href="portfolio.jsp">SHOWS</a></li>
						</ul>
					</li>
	...
	<div class="visual-area works">
		<div class="visual-area-in">
			<h2>SHOWS</h2>
			<div class="location">
				<p><a href="../main/main.jsp"><img src="../../../img/User/com/ico_home.png" alt="HOME" /></a><span>WORKS</span><span>SHOWS</span></p>
			</div>
			<div class="current-location">
				<p><a href="../main/main.jsp"><img src="../../../img/User/com/ico_home_side.png" alt="HOME" /></a><span>WORKS</span><span>SHOWS</span></p>
			</div>
	...
	<div id="container">
		<div class="content">
			<div class="tab">
				<ul>
					<li ><a href="Portfolio.do">전체</a></li>
					<li  ><a href="PortfolioOn.do">방영</a></li>
					<li  ><a href="PortfolioEnd.do">종영</a></li>
					<li class="on" ><a href="PortfolioGlo.do">글로벌</a></li>
				</ul>
			</div>
...
<%} %>
	...
	<!-- E : container -->
	<form name="form" id="form" action="http://www.studiodragon.net/front/kr/works/get">
		<input type="hidden" name="class1" id="class1" value="G"/>
	...
	function fnView(idx) {
		$("#idx").val(idx);
		$("#form").attr("action", "/front/kr/works/portfolio_view");
	...
	function fn_ajaxSearch(pageFlag){
		$.ajax({
			url: "/front/kr/works/portfolioAjax",
			type: "POST",
			data: {
						"class1" 	: $("#class1").val()
						, "cpage"	: $("#cpage").val()
			...
			success: function(data){
				...
				if (data.list.length > 0) {
					for(var i=0;i<data.list.length;i++){
						innerHtml +='<li>';
						//innerHtml +='<a href="/front/kr/works/portfolio_view?class1=G&idx='+data.list[i].idx+'" target="_self">';
						innerHtml +='<a href="javascript:void(0)" onclick="fnView('+data.list[i].idx+')">';
						if (data.list[i].thum_img != null && data.list[i].thum_img != "") {
							innerHtml +='<img src="studio2\images.studiodragon.net\upload\works\kr'+data.list[i].thum_img+'" alt="'+data.list[i].title+' 포스터 이미지" class="listImg" />';
						} else {
							innerHtml +='<img src="/resources/front/kr/images/works/portfolio_list_blank.jpg" alt="'+data.list[i].title+' 포스터 이미지" class="listImg" />';
						}
						innerHtml +='<div class="dimLayer">';

						innerHtml +='<dl>';
						innerHtml +='<dt>'+data.list[i].title+'</dt>';
						innerHtml +='<dd>';
						innerHtml +='<ul>';
						innerHtml += '<li><strong>연출</strong> <span>'+data.list[i].direction+'</span></li>';
						innerHtml += '<li><strong>극본</strong> <span>'+data.list[i].play+'</span></li>';
						innerHtml += '<li><strong>출연</strong> <span>'+data.list[i].actor+'</span></li>';
						innerHtml +='</ul>';

						innerHtml +='</dd>';
						innerHtml +='</dl>';
						innerHtml +='</div>';
						innerHtml +='<p class="tit">'+data.list[i].title+'</p>';

						if(data.list[i].airYn=='Y'){
							innerHtml+='<strong class="icoOnAir">ON-AIR</strong>';
						}
					...
					if (parseInt(data.list[0].totalCount) > (parseInt(data.info.cpage)*6) ){
						$("#btnMore").show();
					} else {
						$("#btnMore").hide();
					}
				} else {
					innerHtml ='<li class="noData">등록된 내용이 없습니다.</li>';
				}
				var $listLast = $("#dataUl").find('li:last-child');
				$("#dataUl").append(innerHtml);
				$listLast.next().find('a:first-child').attr('tabindex','0');
				$listLast.next().find('a:first-child').focus();
			},
			error		:
	...
	$(document).ready(function() {
		if (document.location.hash) {
			var hashPage = document.location.hash.replace("#","");
			$("#cpage").val(hashPage);
			fn_ajaxSearch("M");
		} else {
			var pageFlag = "";
